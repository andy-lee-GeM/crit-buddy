...
class PipeCrossModel(OMCModel):
    ...
    def build_model(self, p):
        ...
        materials, m_fuel, m_gas, m_wall, m_water = self.create_materials(p)
    ...
    def create_plots(self, dims, materials):
        ...
        plot_xy.basis = "xy"
        plot_xy.origin = (
            0.5 * (dims["X_MIN_CM"] + dims["X_MAX_CM"]),
            0.5 * (dims["Y_MIN_CM"] + dims["Y_MAX_CM"]),
            0.0,
        )
        ...
        plot_xz.origin = (
            0.5 * (dims["X_MIN_CM"] + dims["X_MAX_CM"]),
            0.0,
            0.5 * (dims["Z_MIN_CM"] + dims["Z_MAX_CM"]),
        )
